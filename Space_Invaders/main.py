...
while True:
    ...
    for event in pygame.event.get():
        ...
        if keys[pygame.K_SPACE] and game.run == False:
            game.reset()
        if keys[pygame.K_ESCAPE]:  # Check for "Esc" key press
            pygame.quit()
            sys.exit()

        if len(game.aliens_group) == 0:
            game.run = False  # Stop the game
            # Add your "You won" message display here
            win_message_surface = font.render("You Won!", False, YELLOW)
            screen.blit(win_message_surface, (570, 740, 50, 50))
